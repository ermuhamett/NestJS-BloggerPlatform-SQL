...
export class EmailService {
  ...
  async sendRegistrationEmail(userEmail: string, confirmationCode: string) {
    const emailTemplate = this.getRegistrationMailTemplate(
      userEmail,
      confirmationCode,
    );
  ...
  async sendPasswordRecoveryEmail(userEmail: string, recoveryCode: string) {
    const emailTemplate = this.getPasswordRecoveryMailTemplate(
      userEmail,
      recoveryCode,
    );
    return await this.sendEmail(emailTemplate);
  ...
  private getRegistrationMailTemplate(
    userEmail: string,
  ...
  private getPasswordRecoveryMailTemplate(
    userEmail: string,
    recoveryCode: string,
  ) {
    return {
